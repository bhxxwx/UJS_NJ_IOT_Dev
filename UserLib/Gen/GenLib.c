...
static QUEUE8_t m_QueueComRx[4] = { 0 };//链表数据记录结构体，记录了链表的长度，地址等
static uint8_t m_ComRxBuf[4][1000] = { 0 };//串口数据接收缓冲区链表
...
//__attribute__ ((alias("usart_printf"))) int USART_printf(USART_TypeDef* USARTx, const char *fmt,
//        ...);
...
void usart_1_init(int bandrate)
{
	...
	QUEUE_PacketCreate(&m_QueueComRx[0], m_ComRxBuf[0], sizeof(m_ComRxBuf[0]));//链表初始化
}
...
void usart_2_init(int bandrate, char nvic_pree)
{
	...
	QUEUE_PacketCreate(&m_QueueComRx[1], m_ComRxBuf[1], sizeof(m_ComRxBuf[1]));//链表初始化
}
...
void usart_3_init(int bandrate, char nvic_pree)
{
	...
	QUEUE_PacketCreate(&m_QueueComRx[2], m_ComRxBuf[2], sizeof(m_ComRxBuf[2]));//链表初始化
}
 ...
 * nvic_pree:抢占优先级4位,可进行嵌套,可选值为:(高优先级)0~15(低优先级)
 */
void usart_4_init(int bandrate, char nvic_pree)
{
	GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
	...
	USART_ITConfig(UART4, USART_IT_RXNE, ENABLE);	//打开接收到数据中断
	USART_Cmd(UART4, ENABLE);
	QUEUE_PacketCreate(&m_QueueComRx[3], m_ComRxBuf[3], sizeof(m_ComRxBuf[3]));//链表初始化
}
...
void USART1_IRQHandler(void)
{
	...
	{
		USART_ReceiveData(USART1);//清除ORE错误标记位
		return;
	}
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)	//接收到数据
	{
		uint8_t pbuff = USART_ReceiveData(USART1);
		QUEUE_PacketIn(&m_QueueComRx[0], &pbuff, 1);//将数据入链表
...
void USART2_IRQHandler(void)
{
	...
	{
		USART_ReceiveData(USART2);//清除ORE错误标记位
		return;
	}
	if (USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) //接收到数据
	{
		uint8_t pbuff = USART_ReceiveData(USART2);
		QUEUE_PacketIn(&m_QueueComRx[1], &pbuff, 1);//将数据入链表
...
void USART3_IRQHandler(void)
{
	...
	{
		USART_ReceiveData(USART3);//清除ORE错误标记位
		return;
	}
	if (USART_GetITStatus(USART3, USART_IT_RXNE) != RESET) //接收到数据
	{
		uint8_t pbuff = USART_ReceiveData(USART3);
		QUEUE_PacketIn(&m_QueueComRx[2], &pbuff, 1);//将数据入链表
...
void UART4_IRQHandler(void)
{
	...
	{
		USART_ReceiveData(UART4);//清除ORE错误标记位
		return;
	}
	if (USART_GetITStatus(UART4, USART_IT_RXNE) != RESET) //接收到数据
	{
		uint8_t pbuff = USART_ReceiveData(UART4);
		QUEUE_PacketIn(&m_QueueComRx[3], &pbuff, 1);//将数据入链表
 ...
 * 串口发送数据
 * @param USARTx 发送的串口
 * @param data 需要发送的数据的uint8_t型指针
 * @param length 发送的数据位数
 */
void usart_send(USART_TypeDef* USARTx,uint8_t *data, int length)
{
	int i = 0;
	for (; i < length; i++)
	{
...
/**
 * 串口数据读取
 * @param UARTx 需要读取的串口,值为1...4
 * @param buffter 数据读取后存放的地址
 * @param buffterSize 读取的数据长度
 * @return 函数返回本次从链表缓冲区中读取的实际数据数量
 */
uint32_t UsartxRead(uint8_t UARTx,uint8_t *buffter, uint32_t buffterSize)
{
	return QUEUE_PacketOut(&m_QueueComRx[UARTx-1], buffter, buffterSize);
}

/**
 * 串口链表数据缓冲区已缓存的数据数量
 * @param UARTx 需要读取的串口,值为1...4
 * @return 返回当前缓冲区未读取的数据数量
 */
uint32_t UsartxRxAvailable(uint8_t UARTx)
{
	return QUEUE_PacketLengthGet(&m_QueueComRx[UARTx-1]);
}

QUEUE8_t* GetQueue(uint8_t UARTx)
{
	return &m_QueueComRx[UARTx-1];
}
 ...
 **---------------------------------------------------------------------------
 */
int _formatlength(const char *fmt, va_list va)
{
	int length = 0;
	while (*fmt)
	{
 ...
 **---------------------------------------------------------------------------
 */
int _formatstring(char *buf, const char *fmt, va_list va)
{
	char *start_buf = buf;
	while (*fmt)
	{
...
int usart_printf(USART_TypeDef* USARTx, const char *fmt, ...)
{
	...
	va_list va;
	va_start(va, fmt);
	length = _formatlength(fmt, va);
	va_end(va);
	{
		char buf[length];
		va_start(va, fmt);
		length = _formatstring(buf, fmt, va);
